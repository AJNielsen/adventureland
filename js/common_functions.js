...
var REPS = (Number && Number.EPSILON) || commonFunctions.EPS;
var CINF = 999999999999999;
var really_old = new Date();
really_old.setYear(1970);
var colors = {
	range: "#93A6A2",
	armor: "#5C5D5E",
	resistance: "#6A5598",
...
function process_game_data() {
	...
	for (var name in G.monsters) {
		...
		if (G.monsters[name].speed >= 60) G.monsters[name].charge = round(G.monsters[name].speed * 1.2);
		else if (G.monsters[name].speed >= 50) G.monsters[name].charge = round(G.monsters[name].speed * 1.3);
	...
	for (var c in G.classes) {
		...
		free_cx.forEach(function (cx) {
			if (!G.classes[c].xcx.includes(cx)) G.classes[c].xcx.push(cx);
		});
	...
	for (var name in G.maps) {
		...
		(map.npcs || []).forEach(function (npc) {
			...
			var coords = { map: name, in: name, x: npc.position[0], y: npc.position[1], id: npc.id },
				data = G.npcs[npc.id];
			if (data.items) {
	...
	for (var id in G.items) G.items[id].id = id;
}
...
function map_cx(player) {
	...
	for (var c in (player.p && player.p.acx) || player.acx || {}) {
		if (G.cosmetics.bundle[c])
			G.cosmetics.bundle[c].forEach(function (cc) {
				cx[cc] = c;
			});
		else if (G.cosmetics.map[c]) cx[G.cosmetics.map[c]] = c;
		else cx[c] = c;
	}
	return cx;
}
...
function all_cx(player, send) {
	...
	for (var c in (player.p && player.p.acx) || player.acx || {}) {
		...
		else if (G.cosmetics.map[c]) cx[G.cosmetics.map[c]] = (cx[G.cosmetics.map[c]] || 0) + 1;
		else cx[c] = (cx[c] || 0) + 1;
	}
	if (send) {
	...
	((player.p && player.p.xcx) || player.xcx || []).forEach(function (c) {
		...
		else if (G.cosmetics.map[c]) cx[G.cosmetics.map[c]] = (cx[G.cosmetics.map[c]] || 0) + 0.01;
		else cx[c] = (cx[c] || 0) + 0.01;
	});
...
function prune_cx(cx, skin) {
	...
	for (var sname in cx) {
		if (
			!cx[sname] ||
			(sname == "upper" && T[cx[sname]] != "body" && T[cx[sname]] != "armor") ||
			(sname != "upper" && cxtype_to_slot[T[cx[sname]]] != sname)
		) {
			delete cx[sname];
			continue;
		}
...
function hardcore_logic() {
	...
	for (var id in G.monsters) {
		if (G.monsters[id].respawn != -1) G.monsters[id].respawn = 1;
	}
...
function can_stack(a, b, d, args) {
	if (
		...
		G.items[a.name].s &&
		a.name == b.name &&
		a.q + b.q + (d || 0) <= ((G.items[a.name].s === true && 9999) || G.items[a.name].s)
	) {
...
function can_add_item(player, new_item, args) {
	// long requested feature [18/10/18]
	if (!args) args = {};
...
function push_deferred(name) {
	...
	if (["attack", "heal"].includes(name)) current.start = new Date();
...
function resolve_deferred(name, data) {
	...
	if (!deferreds[name] || !deferreds[name].length)
		return (
			console.error("Weird resolve_deferred issue: " + name),
	...
	if (0) {
		// bad idea to start with, regular pings to the server is a better idea [08/10/21]
		if (deferreds[name] && deferreds[name].length && deferreds[name][deferreds[name].length - 1].start)
			push_ping(mssince(deferreds[name][deferreds[name].length - 1].start));
		// if there's a promise bug, this covers the .ping outbreak issue
...
function reject_deferred(name, data) {
	...
	if (!deferreds[name] || !deferreds[name].length)
		return (
			console.error("Weird reject_deferred issue: " + name),
			console.log("If you emit socket events manually, ignore this message")
		);
	current_deferred = deferreds[name].shift();
	if (!deferreds[name].length) delete deferreds[name];
...
function within_xy_range(observer, entity) {
	...
	var x = get_x(entity),
		...
		o_y = get_y(observer);
	if (
		o_x - observer.vision[0] < x &&
		x < o_x + observer.vision[0] &&
		o_y - observer.vision[1] < y &&
		y < o_y + observer.vision[1]
	)
		return true;
	return false;
}
...
function distance(_a, _b, in_check) {
	...
	// Check overlap
	if (a_x - a_w2 <= b_x + b_w2 && a_x + a_w2 >= b_x - b_w2 && a_y >= b_y - b_h && a_y - a_h <= b_y) return 0;
...
function random_away(x, y, R) {
	// https://stackoverflow.com/a/5838055/914546
...
function is_disabled(entity) {
	if (
		...
		entity.rip ||
		(entity.s &&
			(entity.s.stunned || entity.s.fingered || entity.s.stoned || entity.s.deepfreezed || entity.s.sleeping))
	)
		return true;
}
...
function calculate_item_value(item, m) {
	...
	var def = G.items[item.name] || G.items.placeholder_m,
		value = (def.cash && def.g) || def.g * (m || 0.6),
...
function dps_multiplier(defense) {
	// [10/12/17]
	return (
		1 -
		(max(0, min(100, defense)) * 0.001 +
...
function to_pretty_float(num) {
	...
	if (!num) return "0";
	var hnum = floor_f2(num).toFixed(2),
		num = parseFloat(hnum);
...
function calculate_vxy(monster, speed_mult) {
	...
	if (1 || is_game)
		monster.angle = (Math.atan2(monster.going_y - monster.from_y, monster.going_x - monster.from_x) * 180) / Math.PI; // now the .angle is used on .resync [03/08/16]
...
function calculate_movex(map, cur_x, cur_y, target_x, target_y) {
	...
	var XEPS = 10 * commonFunctions.EPS; // 1 EPS isn't enough, can's move along line[0]+EPS with can_move
	...
	for (var i = bsearch_start(x_lines, min_x); i < x_lines.length; i++) {
		...
		if (eps_equal(cur_x, target_x) && eps_equal(cur_x, line_x)) {
			// allows you to move parallelly right into the lines
			line_xE = line_x;
			if (going_down)
				(y_intersect = min(line[1], line[2]) - XEPS), (target_y = min(target_y, y_intersect)), (max_y = target_y);
	...
	for (var i = bsearch_start(y_lines, min_y); i < y_lines.length; i++) {
		...
		if (eps_equal(cur_y, target_y) && eps_equal(cur_y, line_y)) {
			line_yE = line_y;
			if (going_right)
				(x_intersect = min(line[1], line[2]) - XEPS), (target_x = min(target_x, x_intersect)), (max_x = target_x);
...
function set_base(entity) {
	...
	if (G.dimensions[type] && G.dimensions[type][3]) {
		...
		entity.base.h = min(12, (get_width(entity) || 0) * 0.8);
		entity.base.v = min(9.9, (get_height(entity) || 0) / 4.0);
		if (!entity.base.h) (entity.base.h = 8), (entity.base.v = 7);
	}
}

function calculate_move_v2(map, cur_x, cur_y, target_x, target_y) {
	// improved, v2 - all movements should originate from cur_x and cur_y
	if (target_x == Infinity) target_x = CINF;
	if (target_y == Infinity) target_y = CINF;
	var move = calculate_movex(map, cur_x, cur_y, target_x, target_y);
	if (move.x != target_x && move.y != target_y) {
		// this is a smooth move logic - if a line hit occurs, keeps moving in the movable direction
...
var m_calculate = false,
	...
	line_hit_y = null,
	m_dx,
	m_dy; // flags so can_calculate and can_move work in synergy

function calculate_move(entity, target_x, target_y) {
	// v5, calculate 4 edges, choose the minimal move [18/07/18]
	...
	if (entity.base)
		corners = [
			[-entity.base.h, entity.base.vn],
			[entity.base.h, entity.base.vn],
			[-entity.base.h, -entity.base.v],
			[entity.base.h, -entity.base.v],
		];
	...
	if (point_distance(cur_x, cur_y, move.x, move.y) < 10 * commonFunctions.EPS) move = { x: cur_x, y: cur_y }; // The new movement has a bouncing effect, so for small moves, just don't move
	m_calculate = false;
	return move;
}
...
function can_move(monster, based) {
	...
	if (!based && monster.base) {
		// If entity is a rectangle, check all 4 corner movements
		var can = true;
		[
			...
			var mx = mxy[0],
				my = mxy[1];
			if (!can || !can_move({ map: monster.map, x: x0 + mx, y: y0 + my, going_x: x1 + mx, going_y: y1 + my }, 1))
				can = false;
		});
		if (1) {
			...
			if (!can || !can_move({ map: monster.map, x: x1 + px1, y: y1 + py0, going_x: x1 + px1, going_y: y1 + py1 }, 1))
				can = false;
			if (!can || !can_move({ map: monster.map, x: x1 + px0, y: y1 + py1, going_x: x1 + px1, going_y: y1 + py1 }, 1))
				can = false;
	...
	function line_hit_logic(ax, ay, bx, by) {
		(line_hit_x = m_line_x(ax, bx)),
			(line_hit_x = m_line_x(line_hit_x + 6 * commonFunctions.EPS, line_hit_x - 6 * commonFunctions.EPS) + m_dx);
		(line_hit_y = m_line_y(ay, by)),
			(line_hit_y = m_line_y(line_hit_y + 6 * commonFunctions.EPS, line_hit_y - 6 * commonFunctions.EPS) + m_dy);
	}
	for (var i = bsearch_start(GEO.x_lines || [], minx); i < (GEO.x_lines || []).length; i++) {
		if (is_server) perfc.roam_ops += 1;
		var line = GEO.x_lines[i]; // c++;
		if (line[0] == x1 && ((line[1] <= y1 && line[2] >= y1) || (line[0] == x0 && y0 <= line[1] && y1 > line[1]))) {
			// can't move directly onto lines - or move over lines, parallel to them
		...
		if (!(line[1] - commonFunctions.EPS <= next && next <= line[2] + commonFunctions.EPS)) continue; // Fixed EPS [16/07/18]
		//add_log("line clash")
		if (m_line_y) line_hit_logic(line[0], line[1], line[0], line[2]);
		return false;
	}
	for (var i = bsearch_start(GEO.y_lines || [], miny); i < (GEO.y_lines || []).length; i++) {
		...
		if (!(line[1] - commonFunctions.EPS <= next && next <= line[2] + commonFunctions.EPS)) continue;
		if (m_line_x) line_hit_logic(line[1], line[0], line[2], line[0]);
		return false;
	}
	// console.log(c);
...
function unstuck_logic(entity) {
	// handles the case where you can blink onto a line
	if (
		!can_move({
			map: entity.map,
			x: get_x(entity),
			y: get_y(entity),
			going_x: get_x(entity),
			going_y: get_y(entity) + commonFunctions.EPS / 2,
			base: entity.base,
		})
	) {
		var fixed = false;
		if (
			can_move({
				...
				y: get_y(entity) + 8.1,
				going_x: get_x(entity),
				going_y: get_y(entity) + 8.1 + commonFunctions.EPS / 2,
				base: entity.base,
			})
		) {
			set_xy(entity, get_x(entity), get_y(entity) + 8.1);
			fixed = true;
		} else if (
			can_move({
				map: entity.map,
...
function is_door_close(map, door, x, y) {
	var def = G.maps[map],
		spawn = def.spawns[door[6]];
	if (point_distance(x, y, spawn[0], spawn[1]) < 40) return true;
...
function can_use_door(map, door, x, y) {
	// this one is costly, so only check when is_door_close [14/09/18]
	var def = G.maps[map],
		spawn = def.spawns[door[6]];
	if (
		point_distance(x, y, spawn[0], spawn[1]) < 40 &&
		can_move({ map: map, x: x, y: y, going_x: spawn[0], going_y: spawn[1] })
	) {
		//console.log("SPAWNACCESS"+spawn);
		return true;
	}
	if (
		distance({ x: x, y: y, width: 26, height: 35 }, { x: door[0], y: door[1], width: door[2], height: door[3] }) < 40
	) {
		var can = false;
		[
			[0, 0],
...
function is_point_inside(p, polygon) {
	...
	for (i, j; i < polygon.length; j = i++) {
		if (
			...
			p[0] <
				((polygon[j][0] - polygon[i][0]) * (p[1] - polygon[i][1])) / (polygon[j][1] - polygon[i][1]) + polygon[i][0]
		) {
			isInside = !isInside;
		}
	}
...
function delete_indices(array, to_delete) {
	...
	for (var i = to_delete.length - 1; i >= 0; i--) array.splice(to_delete[i], 1);
}
...
function in_arr(i, kal) {
	if (is_array(i)) {
		for (var j = 0; j < i.length; j++) for (var el in kal) if (i[j] === kal[el]) return true;
	}
...
function eps_equal(a, b) {
	return Math.abs(a - b) < 5 * commonFunctions.EPS;
}
...
function lstack(arr, el, limit) {
	// used for logging purposes, last entry becomes first element
	arr.unshift(el);
...
function Heap(array, type, compareFunction) {
	...
	var x = {
		...
		_bubbleUp: function (index) {
			...
			if (
				(this.type === TYPE_MIN && this.compare(value, parent) < 0) ||
